...
namespace WPM {
	public partial class WorldMapEditor : MonoBehaviour {
		...
		public string[] countryNeighboursNames {
			get {
				if (_countryNeighboursNames==null) ReloadCountryNeighboursNames();
		...
		public bool CountryChangeContinent () {
			if (countryIndex<0 || countryIndex>=map.countries.Length) return false;
		...
		bool GetCountryIndexByGUISelection() {
			if (GUICountryIndex<0 || GUICountryIndex>=_countryNames.Length) return false;
		...
		public void CountryRegionSelect() {
			...
			if (countryTargetIndex==countryIndex) GUICountryTransferToCountryIndex = -1;
			if (GUICountryTransferToCountryIndex<0 && _countryNeighboursNames.Length!=_countryNames.Length) GUICountryTransferToCountryIndex = 1; // != means there's a neighbour section at top
		}
...
//			}
		...
		public void ReloadCountryNeighboursNames () {
			if (countryIndex<0 || _countryNames==null) return;
		...
		public void ReloadProvinceCountriesNeighboursNames () {
			if (provinceIndex<0 || _countryNames==null) return;
		...
		string GetCountryTransferIndexByGUISelection() {
			if (_countryNames==null || GUICountryTransferToCountryIndex<0 || GUICountryTransferToCountryIndex>=_countryNames.Length) return "";
...
//			countryChanges = true;
		...
		public void CountrySanitize() {
			...
			for (int k=0;k<rcount;k++) {
				...
				if (_map.RegionSanitize(region)) changes = true;
			}
			if (changes) {
		...
		public void CountryTransferTo() {
			if (countryIndex<0 || GUICountryTransferToCountryIndex<0) return;
			// Get target country
			// recover GUI country index selection
		...
		int GetCountryTransferToIndex() {
			...
			if (GUICountryTransferToCountryIndex<0 || _countryNeighboursNames==null) return -1;
		...
		public string GetCountryGeoData () {
			...
			for (int k=0; k<map.countries.Length; k++) {
				...
				if (country.regions.Count<1) continue;
				if (k > 0)
					sb.Append ("|");
