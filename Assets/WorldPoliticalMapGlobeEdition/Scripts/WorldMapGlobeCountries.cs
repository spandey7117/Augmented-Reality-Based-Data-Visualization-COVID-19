...
namespace WPM {
	...
	/* Event definitions */
	public delegate void OnCountryBeforeEnter(int countryIndex, int regionIndex, ref bool ignoreCountry);
	public delegate void OnCountryEvent(int countryIndex, int regionIndex);

	/* Public WPM Class */
	public partial class WorldMapGlobe : MonoBehaviour {

		public event OnCountryBeforeEnter OnCountryBeforeEnter;
		...
		public Country[] countries {
			...
			set { _countries = value; lastCountryLookupCount = -1; }
		}
		...
		[SerializeField]
		LABELS_RENDER_METHOD _labelsRenderMethod = LABELS_RENDER_METHOD.Blended;
...
//		}
		...
		public int GetCountryNearPoint(Vector3 spherePosition) {
			...
			for (int k=0;k<countryCount;k++) {
				...
				float dist = Vector3.SqrMagnitude(country.sphereCenter - spherePosition);
				if (dist<minDist) {
					minDist = dist;
					countryIndex = k;
				}
		...
		public void FlyToCountry (int countryIndex, float duration) {
			...
			FlyToLocation(countries [countryIndex].sphereCenter, duration);
		}
		...
		public void ToggleCountryMainRegionSurface(int countryIndex, bool visible, Color color) {
			ToggleCountryMainRegionSurface(countryIndex, visible, color, _showOutline, _outlineColor);
		}
		...
		public void ToggleCountryMainRegionSurface(int countryIndex, bool visible, Texture2D texture) {
			ToggleCountryMainRegionSurface(countryIndex, visible, Color.white, texture, Misc.Vector2one, Misc.Vector2zero, 0);
		}
		...
		/// Colorize main region of a country by index in the countries collection.
		/// </summary>
		public void ToggleCountryMainRegionSurface(int countryIndex, bool visible, Color color, bool drawOutline, Color outlineColor) {
			ToggleCountryRegionSurface(countryIndex, countries[countryIndex].mainRegionIndex,visible, color, null, Misc.Vector3one, Misc.Vector3zero, 0, drawOutline, outlineColor);
		}
		...
		public void ToggleCountryMainRegionSurface(int countryIndex, bool visible, Color color, Texture2D texture, Vector2 textureScale, Vector2 textureOffset, float textureRotation) {
			ToggleCountryMainRegionSurface(countryIndex, visible, color, texture, textureScale, textureOffset, textureRotation, _showOutline, _outlineColor);
		}
		...
		/// </summary>
		/// <param name="texture">Optional texture or null to colorize with single color</param>
		public void ToggleCountryMainRegionSurface(int countryIndex, bool visible, Color color, Texture2D texture, Vector2 textureScale, Vector2 textureOffset, float textureRotation, bool drawOutline, Color outlineColor) {
			ToggleCountryRegionSurface(countryIndex, countries[countryIndex].mainRegionIndex,visible, color, texture, textureScale, textureOffset, textureRotation, drawOutline, outlineColor);
		}
		...
		public void ToggleCountryRegionSurface(int countryIndex, int regionIndex, bool visible, Color color) {
			ToggleCountryRegionSurface(countryIndex, regionIndex,visible, color, null, Misc.Vector3one, Misc.Vector3zero, 0);
		}
		...
		public void ToggleCountryRegionSurface (int countryIndex, int regionIndex, bool visible, Color color, Texture2D texture, Vector2 textureScale, Vector2 textureOffset, float textureRotation) {
			ToggleCountryRegionSurface(countryIndex, regionIndex, visible, color, texture, textureScale, textureOffset, textureRotation, _showOutline, _outlineColor);
		}

		/// <summary>
		/// Colorize specified region of a country by indexes.
		/// </summary>
		public void ToggleCountryRegionSurface (int countryIndex, int regionIndex, bool visible, Color color, Texture2D texture, Vector2 textureScale, Vector2 textureOffset, float textureRotation, bool drawOutline, Color outlineColor) {
			if (!visible) {
			...
			if (surf!=null) {
				...
				surf = GenerateCountryRegionSurface (countryIndex, regionIndex, surfMaterial, textureScale, textureOffset, textureRotation, drawOutline, outlineColor);
		...
		public GameObject ToggleCountryRegionSurfaceHighlight (int countryIndex, int regionIndex, Color color, bool drawOutline) {
			...
			if (existsInCache) {
				...
				surf = GenerateCountryRegionSurface (countryIndex, regionIndex, mat, Misc.Vector2one, Misc.Vector2zero, 0, drawOutline, _outlineColor);
			}
			return surf;
		}
		...
		public void BlinkCountry (int countryIndex, int regionIndex, Color color1, Color color2, float duration, float blinkingSpeed) {
			BlinkCountry(countryIndex, regionIndex, color1, color2, duration, blinkingSpeed, _showOutline, _outlineColor);
		}

		/// <summary>
		/// Flashes specified country's region.
		/// </summary>
		public void BlinkCountry (int countryIndex, int regionIndex, Color color1, Color color2, float duration, float blinkingSpeed, bool drawOutline, Color outlineColor) {
			int cacheIndex = GetCacheIndexForCountryRegion (countryIndex, regionIndex);
			GameObject surf;
			bool disableAtEnd;
			if (surfaces.ContainsKey (cacheIndex)) {
				...
				disableAtEnd = !surf.activeSelf;
					...
					surf = GenerateCountryRegionSurface (countryIndex, regionIndex, hudMatCountry, drawOutline, outlineColor);
				disableAtEnd = true;
			}
				...
				if (d<-0.2f) {
					...
					if (vpos.x>=0 && vpos.x<=1 && vpos.y>=0 && vpos.y<=1) {
						...
						for (int p=0;p<frontier.Length;p+=step) {
							...
							vpos = Camera.main.WorldToViewportPoint(pos);
							if (vpos.x>=0 && vpos.x<=1 && vpos.y>=0 && vpos.y<=1) {
								vc.Add(country);
								break;
							}
		...
		public List<Country>GetVisibleCountries(Vector3 rectTopLeft, Vector3 rectBottomRight) {
			...
			for (int k=0;k<countries.Length;k++) {
				Country country = countries[k];
				if (country.hidden) continue;
				if (selectedCountries.Contains(country)) continue;
		...
		public bool CountryTransferCountryRegion(int countryIndex, Region sourceCountryRegion) {
			...
			if (countryIndex<0 || sourceCountryIndex<0 || countryIndex == sourceCountryIndex) return false;
			...
			for (int k=0;k<cityCount;k++) {
				if (cities[k].countryIndex == sourceCountryIndex) cities[k].countryIndex = countryIndex;
			}
			
			// Transfer mount points
			int mountPointCount = mountPoints.Count;
			for (int k=0;k<mountPointCount;k++) {
				if (mountPoints[k].countryIndex == sourceCountryIndex) mountPoints[k].countryIndex = countryIndex;
			}
			...
			bool independentRegion = true;
			if (sourceCountryRegion.Intersects(targetRegion)) {
				RegionMagnet(sourceCountryRegion, targetRegion);
				PolygonClipper pc = new PolygonClipper(targetRegion, sourceCountryRegion);
				independentRegion = !pc.Compute(PolygonOp.UNION, null);
			}
			...
			if (countryIndex>sourceCountryIndex) countryIndex--;
		...
		public bool CountryTransferProvinceRegion(int targetCountryIndex, Region provinceRegion) {
			...
			if (provinceIndex<0 || targetCountryIndex<0 || targetCountryIndex>=countries.Length) return false;
			...
			if (sourceCountryIndex == targetCountryIndex) return false;
			...
			if (sourceCountry.regions.Count==0) {
				internal_CountryDelete(sourceCountryIndex, false);
				if (targetCountryIndex>sourceCountryIndex) targetCountryIndex--;
			} else {
